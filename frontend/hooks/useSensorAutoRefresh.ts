...
import { addSensorDataById, setErrorById } from "@/store/slices/sensorSlice";
import type { SensorId } from "@/store/slices/sensorSlice";
import { fetchDataByType, groupBySensorId, getSensorPrefix, type DataType } from "@/lib/api/sensorApi";
    ...
    const refreshData = async () => {
      ...
      const promises = dataTypes.map(async (dataType) => {
        try {
          ...
          console.error(`Błąd odświeżania danych dla ${dataType}:`, errorMessage);
